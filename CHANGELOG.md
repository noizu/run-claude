...
## [Unreleased]
- `zai-pro` / `zai-alt` haiku tier now routes through `glm-5.3-flashx` (was `glm-5-turbo`/named `haiku` alias): `zai-pro` haiku_model → `zai/glm-5.3-flashx (sub)` (matching the `(sub)` SKU form already used by its opus/sonnet tiers); `zai-alt` haiku_model → `zai-alt/glm-5.3-flashx` (auto-synthesized family clone).
