...
## [Unreleased]
- `zai-pro` / `zai-alt` haiku tier now routes through `glm-5.3-flash` (was the named `haiku` alias on `glm-5-turbo`): `zai-pro` haiku_model → `zai/glm-5.3-flash (sub)` (matching the `(sub)` SKU form already used by its opus/sonnet tiers); `zai-alt` haiku_model → `zai-alt/glm-5.3-flash` (auto-synthesized family clone). The faster `glm-5.3-flashx` stays confined to the dedicated `zai-pro-x` / `zai-alt-x` profiles.
...
- `qwen` profile on Alibaba Token Plan SKUs: fable=`alibaba/qwen3.8-max`, opus=`alibaba/deepseek-v4-pro-0813`, sonnet=`alibaba/qwen3.8-flash`, haiku=`alibaba/deepseek-v4-flash-0731`. New catalog entries `alibaba/qwen3.8-flash` and `alibaba/deepseek-v4-pro-0813`.
- `zai-pro` tiers: fable=`glm-5.3` (`reasoning_effort: max`), opus=`glm-5.3-flash` (`high`; more capable than 5.2), sonnet=`glm-5.2` (`high`), haiku remains `glm-5-turbo` (`low`). New `zai/fable` and `zai-tyna/fable` aliases plus `[1m]` variants. Same GLM IDs on the `zai/*` family used by kitchen-sink / claude-plan / all-sub / claude-enhanced.
- Named Z.AI SKUs on both protocols: Anthropic `zai/glm-5.3`, `zai/glm-5.3-flash`, `zai/glm-5.2`; coding-plan `zai-oa/glm-5.3-flash`, `zai-oa/glm-5.2`, `zai-oa/fable`. Added to zai-pro / claude-plan / all-sub / claude-enhanced / kitchen-sink pickers (alongside existing `glm-5.3` / `glm-5-turbo` / `glm-4.7`). `zai-oa` profile tiers now match: fable=`glm-5.3`, opus=`glm-5.3-flash`, sonnet=`glm-5.2`, haiku remains `glm-4.7`.
